...
# This allows us to call the script like: `NODES=2 GPUS_PER_NODE=4 ./cluster_interface.sh --cluster deltaai job ...`
NODES=${NODES:-1}
GPUS_PER_NODE=${GPUS_PER_NODE:-1}

# Defaults for DeltaAI. ACCOUNT has no global default — it must come from
# .env.user (DELTAAI_SLURM_ACCOUNT) or the ACCOUNT= env override.
# DeltaAI only has ghx4 and ghx4-interactive partitions (no preempt yet).
PARTITION=${PARTITION:-ghx4}
CPUS_PER_TASK=${CPUS_PER_TASK:-16}
MEM_PER_GPU=${MEM_PER_GPU:-180G}
TIME=${TIME:-24:00:00}

if [ -z "${ACCOUNT:-}" ]; then
    echo "[ERROR] ACCOUNT is empty. Set DELTAAI_SLURM_ACCOUNT in .env.user or pass ACCOUNT=... before the command." >&2
    exit 1
fi
...
echo "----------------------------------------------------------------"
echo "Submitting Job to DeltaAI"
echo "----------------------------------------------------------------"
...
# Enable requeue for all jobs - allows resumption after time limit hits.
# DeltaAI has no preempt partition yet, so requeue is only for USR1 (time-limit).
...
# --- Requeue Handler for Time Limits ---
requeue_handler() {
    echo "[$(date)] Caught signal: $1 - marking job $SLURM_JOB_ID for requeue"
    scontrol requeue $SLURM_JOB_ID
}

# Install signal handlers (TERM kept for parity with Delta even though DeltaAI has no preempt yet).
trap 'requeue_handler USR1' USR1
trap 'requeue_handler TERM' TERM

echo "[$(date)] Job $SLURM_JOB_ID starting on $(hostname)"
echo "[$(date)] Requeue handler installed - job will auto-resume if it hits the time limit"
